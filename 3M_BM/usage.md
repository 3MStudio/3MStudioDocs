...
2. A PIN prompt opens:
   - If `ox_lib` is present, a nice input dialog is used.
   - Otherwise, the native onscreen keyboard is used.
...
- Restocks **all** items in memory to their `maxStock`.
- Persists stock and updates clients.
- Updates `last_restock_at` meta.

Use this to manually refill the store (for example after a wipe).
...
- After editing `Config.Items`.
- After adding/removing items from the config.
- After migrating to a new DB.

### `/addblackitem <item> <count>`

- Adds `count` units of `item` to the stock of that item in memory.
- Persists new stock.
- Broadcasts updated stock list to all clients.

Examples:

```text
/addblackitem WEAPON_APPISTOL 10
/addblackitem ammobox-9mm 500
```
